        ...
        <div ref={ref} onClick={e => e.stopPropagation()} className='opacity-0 w-full h-full transition-all duration-300 rounded-xl bg-neutral-300 flex justify-between items-center p-8 gap-8 relative'>
                ...
                <div className=' flex gap-2 items-center justify-center'>
                    {
                        data.image.map((res, i) => {
                            if (i === imageID){
                                return <div key={i} className='opacity-100 transition-all duration-300 rounded-full leading-3 bg-darkgray w-3 h-3'></div>
                            } else{
                                return <div key={i} className='opacity-30 transition-all duration-300 rounded-full text-sm bg-darkgray leading-none w-2 h-2'></div>
                            }
                        })
                    }
                </div>
            ...
            <div className=' grow  flex h-full py-12 justify-between flex-col tracking-wide text-darkerblue pr-4'>
                <div>
                    ...
                    <p className=' mt-5 font-light text-lg opacity-80'>{data.desc}</p>
                </div>
